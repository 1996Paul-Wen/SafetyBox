...
# 查询指定用户详情
curl -XPOST 127.0.0.1:7777/api/User/Describe -H "username:abcd"  -H "password:123" -d '{"name": "abcd"}'

# 用户存入档案
curl -XPOST 127.0.0.1:7777/api/SafetyData/Create  -H "username:abcd"  -H "password:123" -d '{"archive_key": "key", "archive_value":"value"}'

# 用户查询档案
curl -XPOST 127.0.0.1:7777/api/SafetyData/List  -H "username:abcd"  -H "password:123" -d '{"archive_key": "key"}'
```